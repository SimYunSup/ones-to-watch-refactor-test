...
같은 코드를 Apple M1 Max에서 클린 빌드한 수치입니다(`pnpm run build:stats`, 빈 컬렉션 — Notion 시크릿 없음). 위 CI 표(ubuntu-latest 2코어, 실콘텐츠 123 포스트)와 비교하면 콘텐츠가 실려도 순수 빌드는 십수 초 이내이고, 콘텐츠 스케일에 가장 민감한 변형은 프리렌더가 페이지 수에 비례하는 TanStack(×5.1)임을 보여줍니다. 이전 CI 수치(RR 312.8s 등)는 빌드가 아니라 Notion 페치 시간이었습니다(현재는 프리페치 캐시로 분리 측정).

| 변형 | 빌드 시간(s) | 총 출력 크기 | JS 크기 | 파일 수 | CI 대비 |
| --- | --- | --- | --- | --- | --- |
| Astro | 2.4 | 769.7 KB | 99.9 KB | 24 | ×3.3 |
| React Router | 2.6 | 593.9 KB | 321.1 KB | 18 | ×3.4 |
| TanStack | 3.1 | 600.1 KB | 332.8 KB | 14 | ×5.1 |
| Kudzu | 0.8 | 284.1 KB | 15.9 KB | 13 | ×1.8 |

_로컬 빌드는 수동 측정(2026-07-23). 출력 크기·파일 수 차이는 콘텐츠 유무 때문이고, JS 크기는 콘텐츠와 무관하게 동일합니다._
