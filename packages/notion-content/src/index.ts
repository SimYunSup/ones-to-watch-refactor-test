...
// Search helpers (htmlToText/entriesToSearchDocs/buildSearchIndexBytes/
// writeSearchIndexFile) and the SearchDoc type live in the "./search"
// subpath export (@otw/notion-content/search), NOT this barrel: they pull in
// @pickhealer/munja's wasm-pack glue, which some app bundlers (Turbopack)
// try to statically resolve and choke on even though only build-time Node
// scripts ever call them. Keeping them off the barrel means importing
// fetchNewsEntries never drags wasm into an app's client/prerender bundle.
